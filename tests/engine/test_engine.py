...
def test_worker_processes_not_ready(monkeypatch):
    """Test _worker_processes_ready returns False when not all workers are ready"""
    cfg = build_cfg()
    engine = LLMEngine.__new__(LLMEngine)
    engine.cfg = cfg
...
def test_format_and_add_data_with_existing_request_id(monkeypatch):
    """Test _format_and_add_data when request_id already exists"""
    cfg = build_cfg()
    engine = LLMEngine.__new__(LLMEngine)
    engine.cfg = cfg

    prompts = {"request_id": "existing_id", "prompt": "test"}

    # Mock add_requests to capture the prompts
    captured = {}

    def mock_add_requests(prompts_dict):
        captured["prompts"] = prompts_dict
        return "req_id"

    engine.add_requests = mock_add_requests

    result = engine._format_and_add_data(prompts)

    assert result == "req_id"
    assert captured["prompts"]["request_id"] == "existing_id"


def test_generate_streaming_continue_on_none_processed(monkeypatch):
    """Test generate streaming continues when process_response returns None"""
    cfg = build_cfg()
    engine = LLMEngine.__new__(LLMEngine)
    engine.cfg = cfg
    engine.data_processor = DummyDataProcessor()
    ...
    # Mock process_response to return None first, then a result
    call_count = 0
    ...
    engine.engine.data_processor.process_response = mock_process_response
    engine._get_generated_tokens = lambda req_id: [DummyResult(False), DummyResult(True)]

    outputs = list(engine.generate({"prompt": "hi"}, stream=True))

    assert len(outputs) >= 1  # Should have at least the final result


def test_launch_components_mixed_mode_cache_manager(monkeypatch):
    """Test launching cache manager in mixed mode"""
    cfg = build_cfg()
    ...
    # Mock platform to not be Intel HPU
    monkeypatch.setattr(engine_module.current_platform, "is_intel_hpu", lambda: False)

    # Mock start_cache_service
    mock_cache_manager = SimpleNamespace()
    monkeypatch.setattr(
        engine_module.EngineService, "start_cache_service", lambda device_ids, suffix: mock_cache_manager
    )

    engine.engine = SimpleNamespace(start_cache_service=lambda device_ids, suffix: mock_cache_manager)
    engine.cache_manager_processes = None

    engine.launch_components()

    assert engine.cache_manager_processes is mock_cache_manager


def test_launch_components_with_zmq_adapter(monkeypatch):
    """Test setting ZMQ ports when internal adapter is enabled"""
    cfg = build_cfg()
    cfg.scheduler_config.splitwise_role = "mixed"
    engine = LLMEngine.__new__(LLMEngine)
    engine.cfg = cfg
    ...
    # Mock envs
    monkeypatch.setattr(envs, "FD_ENABLE_INTERNAL_ADAPTER", True, raising=False)
    monkeypatch.setattr(envs, "FD_ZMQ_RECV_REQUEST_SERVER_PORTS", "8080,8081", raising=False)
    monkeypatch.setattr(envs, "FD_ZMQ_SEND_RESPONSE_SERVER_PORTS", "9090,9091", raising=False)

    engine.launch_components()

    assert envs.FD_ZMQ_RECV_REQUEST_SERVER_PORT == "8080"
    assert envs.FD_ZMQ_SEND_RESPONSE_SERVER_PORT == "9090"


def test_launch_components_dp_path_socket_address(monkeypatch):
    """Test DP path uses socket address instead of TCP"""
    cfg = build_cfg()
    cfg.scheduler_config.name = "dp"
    cfg.parallel_config.data_parallel_size = 2
    cfg.parallel_config.engine_worker_queue_port = [1000, 1001]
    ...
    monkeypatch.setattr(envs, "FD_ENABLE_MULTI_API_SERVER", False, raising=False)
    monkeypatch.setattr(envs, "FD_ENGINE_TASK_QUEUE_WITH_SHM", True, raising=False)
    monkeypatch.setattr(engine_module, "EngineWorkerQueue", lambda **kwargs: DummyQueueServer())
    monkeypatch.setattr(engine_module, "start_data_parallel_service", lambda *args: None)
    monkeypatch.setattr(engine_module.multiprocessing, "Queue", lambda: object())
    monkeypatch.setattr(
        ...
        "get_context",
        lambda name: type("ctx", (), {"Process": lambda **kwargs: DummyProcess(pid=500)})(),
    )
    monkeypatch.setattr(engine_module.time, "sleep", lambda seconds: None)

    engine.launch_components()

    assert engine.dp_engine_worker_queue_server is not None


def test_init_with_intel_hpu_skip_cache_manager(monkeypatch):
    """Test initialization skips cache manager on Intel HPU platform"""
    cfg = build_cfg()
    ...
    # Mock Intel HPU platform
    monkeypatch.setattr(engine_module.current_platform, "is_intel_hpu", lambda: True)

    # Mock other dependencies
    monkeypatch.setattr(
        engine_module, "EngineService", lambda cfg: SimpleNamespace(data_processor=DummyDataProcessor())
    )
    monkeypatch.setattr(engine_module.main_process_metrics, "set_cache_config_info", lambda obj: None)
    monkeypatch.setattr(engine_module.tracing, "trace_set_thread_info", lambda name: None)
    ...
    # Cache manager should not be started on Intel HPU
    assert engine.cache_manager_processes is None


def test_worker_init_status_thread_alive_check(monkeypatch):
    """Test checking if worker init status thread is alive"""
    cfg = build_cfg()
    engine = LLMEngine.__new__(LLMEngine)
    engine.cfg = cfg
    engine.worker_init_status_func_thread = SimpleNamespace(is_alive=lambda: True)

    # Mock time.sleep to avoid actual sleeping
    monkeypatch.setattr(engine_module.time, "sleep", lambda seconds: None)

    # This should return True since thread is alive and within timeout
    result = engine._wait_for_worker_processes_ready()
    assert result is True


def test_check_worker_initialize_status_detects_progress(monkeypatch):
    """Test worker status detection logic for loading progress"""
    cfg = build_cfg()
    engine = LLMEngine.__new__(LLMEngine)
    engine.cfg = cfg
    engine.worker_init_status = {}

    # Mock worker proc with stdout containing progress lines
    stdout_lines = [
        b"Loading checkpoint shards: 50",
        b"Start load layer 2",
    ...
    assert result is True
    assert "weight_loadding" in engine.worker_init_status
    assert "layer_loadding" in engine.worker_init_status


def test_launch_components_cache_manager_sleep(monkeypatch):
    """Test the sleep call during cache manager launch"""
    cfg = build_cfg()
    cfg.parallel_config.data_parallel_size = 2
    engine = LLMEngine.__new__(LLMEngine)
    engine.cfg = cfg
    ...
    sleep_called = []
    monkeypatch.setattr(engine_module.time, "sleep", lambda seconds: sleep_called.append(seconds))

    engine.launch_components()

    # Verify sleep was called with 1 second
    assert 1 in sleep_called


def test_worker_init_status_thread_dead_returns_false(monkeypatch):
    """Test _wait_for_worker_processes_ready returns False when thread dies"""
    cfg = build_cfg()
    engine = LLMEngine.__new__(LLMEngine)
    engine.cfg = cfg

    # Mock thread that dies immediately
    call_count = 0

    def mock_is_alive():
        nonlocal call_count
        call_count += 1
        return call_count == 1  # Alive first time, dead second time

    engine.worker_init_status_func_thread = SimpleNamespace(is_alive=mock_is_alive)
...
def test_check_worker_initialize_status_error_handling(monkeypatch):
    """Test worker init status check handles errors gracefully"""
    cfg = build_cfg()
    engine = LLMEngine.__new__(LLMEngine)
    engine.cfg = cfg
    engine.worker_init_status = {}

    # Mock worker proc that raises exception during status check
    class BadStdout:
        def __iter__(self):
            raise RuntimeError("mock stdout error")

    engine.worker_proc = SimpleNamespace(stdout=BadStdout(), poll=lambda: None)
...
def test_format_and_add_data_generates_new_request_id():
    """Test _format_and_add_data generates new request_id when not provided"""
    cfg = build_cfg()
    engine = LLMEngine.__new__(LLMEngine)
    engine.cfg = cfg
    ...
    def mock_add_requests(prompts_dict):
        captured["prompts"] = prompts_dict
        return "generated_req_id"

    engine.add_requests = mock_add_requests

    result = engine._format_and_add_data(prompts)

    assert result == "generated_req_id"
    assert "request_id" in captured["prompts"]
    assert isinstance(captured["prompts"]["request_id"], str)
