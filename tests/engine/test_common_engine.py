...
class TestCommonEngine(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            with (
                patch("fastdeploy.engine.common_engine.EngineWorkerQueue", TestCommonEngineAdditionalCoverage._make_full_dummy_q_cls()),
...
class TestCommonEngineAdditionalCoverage(unittest.TestCase):
    ...
    def test_stop_profile_waits_until_signal_set(self):
        eng = self._make_mixed_engine()
        eng.ipc_signal_suffix = 9998
        eng.do_profile = 1
        eng.get_profile_block_num_signal = type("Sig", (), {"value": np.array([0], dtype=np.int32)})()
        eng.resource_manager = Mock()
        eng.start_cache_service = Mock(return_value=[Mock()])

        def wake_signal(*args, **kwargs):
            eng.get_profile_block_num_signal.value[0] = 6
            return None

        with patch("fastdeploy.engine.common_engine.time.sleep", side_effect=wake_signal):
            eng._stop_profile()
    ...
    def test_launch_components_splitwise_scheduler_start_called(self):
        eng = self._make_mixed_engine()
        eng.cfg.scheduler_config.name = "splitwise"
        eng.cfg.scheduler_config.splitwise_role = "mixed"
        eng.scheduler = Mock(start=Mock())

        eng.launch_components()

        eng.scheduler.start.assert_called_once()
        self._detach_finalizer(eng)

    def test_check_worker_initialize_status_returns_false_when_worker_exits_in_layer_stage(self):
        eng = self._make_mixed_engine()
        ...
        eng.worker_init_status = {"weight_loadding": 1.0, "layer_loadding": 0.0}
        ...
        with (
            patch("fastdeploy.engine.common_engine.tqdm", lambda *a, **k: DummyPbar()),
            patch.object(eng, "_worker_processes_ready", return_value=False),
    ...
    def test_check_worker_initialize_status_join_exception_ignored(self):
        eng = self._make_mixed_engine()
        ...
        eng.worker_proc = DummyProc()
        eng.worker_init_status = {"weight_loadding": 1.0, "layer_loadding": 1.0}

        with (
            patch("fastdeploy.engine.common_engine.threading.Thread", BadJoinThread),
            patch("fastdeploy.engine.common_engine.tqdm", lambda *a, **k: self._DummyPbar()),
            patch.object(eng, "_worker_processes_ready", return_value=True),
            patch("fastdeploy.engine.common_engine.time.sleep", lambda *_: None),
        ):
            ok = eng.check_worker_initialize_status()
    ...
    def test_register_to_router_logs_http_failure(self):
        eng = self._make_mixed_engine()
        eng.cfg.router_config.router = "http://router"
        ...
        with (
            ...
            patch("fastdeploy.engine.common_engine.requests.post", return_value=Mock(ok=False, status_code=500, text="bad")),
            patch("fastdeploy.engine.common_engine.time.sleep", side_effect=SystemExit),
            patch.object(eng, "llm_logger") as mock_logger,
        ):
            eng._register_to_router()
    ...
    def test_register_to_router_logs_exception(self):
        eng = self._make_mixed_engine()
        eng.cfg.router_config.router = "http://router"
        eng.cfg.router_config.api_server_host = "127.0.0.1"
        eng.cfg.router_config.api_server_port = 8080
        ...
        with (
            patch("fastdeploy.engine.common_engine.threading.Thread", DummyThread),
            patch("fastdeploy.engine.common_engine.check_service_health", return_value=True),
            patch("fastdeploy.engine.common_engine.requests.post", side_effect=fail_post),
            patch("fastdeploy.engine.common_engine.time.sleep", side_effect=one_then_stop),
            patch.object(eng, "llm_logger") as mock_logger,
        ):
            eng._register_to_router()
    ...
    def test_insert_zmq_task_to_scheduler_normal_request(self):
        ...
        with (
            ...
            patch("fastdeploy.engine.common_engine.time.sleep", lambda *_: setattr(eng, "running", False)),
        ):
    ...
    def test_decode_process_splitwise_requests_v1_prefilled_internal_adapter_empty_token(self):
        cfg = self._make_cfg(
            splitwise_role="decode",
            num_gpu_blocks_override=4,
            router="0.0.0.0:30000",
        ...
        eng.token_processor = Mock(tokens_counter={"rid": 1})
        eng.split_connector = Mock(send_cache_info_to_prefill=Mock())
        eng.engine_worker_queue = Mock(
            disaggregate_queue_empty=Mock(side_effect=[False, True]),
            get_disaggregated_tasks=Mock(return_value=[(0, [
                RequestOutput(
                    request_id="rid",
                    outputs=Mock(token_ids=[], decode_type=1, tool_calls=None),
                    metrics=Mock(),
                    finished=False,
                    error_code=200,
                )
        ...
        eng.resource_manager.pre_recycle_resource.assert_called_once_with("rid")
        eng.scheduler.put_results.assert_called()
        self._detach_finalizer(eng)

    def test_decode_process_splitwise_requests_v1_prefilled_error(self):
        cfg = self._make_cfg(
            splitwise_role="decode",
            num_gpu_blocks_override=4,
            router="0.0.0.0:30000",
    ...
    def test_decode_process_splitwise_requests_v1_allocate_success(self):
        cfg = self._make_cfg(splitwise_role="decode", num_gpu_blocks_override=4, router="0.0.0.0:30000")
        eng = self._make_engine(cfg)
        eng.running = True
        eng.enable_decode_cache_task = False
        eng.cfg.splitwise_version = "v1"
        eng.scheduler = Mock(has_request=Mock(return_value=True), put_results=Mock())
        task = Request(request_id="alloc_ok", prompt_token_ids=[1], prompt_token_ids_len=1)
        task.metrics.decode_recv_req_time = 0
        eng.engine_worker_queue = Mock(
            disaggregate_queue_empty=Mock(side_effect=[False, True]),
            get_disaggregated_tasks=Mock(return_value=[(0, [task])]),
        )
        eng.split_connector = Mock(send_cache_info_to_prefill=Mock())
        eng.resource_manager = Mock(preallocate_resource_in_d=Mock(return_value=True))
    ...
    def test_decode_process_splitwise_requests_v1_allocate_fail_without_cache_task(self):
        cfg = self._make_cfg(splitwise_role="decode", num_gpu_blocks_override=4, router="0.0.0.0:30000")
        eng = self._make_engine(cfg)
        eng.running = True
        eng.enable_decode_cache_task = False
        eng.cfg.splitwise_version = "v1"
        eng.scheduler = Mock(has_request=Mock(return_value=True), put_results=Mock())
        task = Request(request_id="alloc_fail", prompt_token_ids=[1], prompt_token_ids_len=1)
        task.metrics.decode_recv_req_time = 0
        eng.engine_worker_queue = Mock(
            disaggregate_queue_empty=Mock(side_effect=[False, True]),
            get_disaggregated_tasks=Mock(return_value=[(0, [task])]),
        ...
        eng.split_connector = Mock(send_cache_info_to_prefill=Mock())
        eng.resource_manager = Mock(preallocate_resource_in_d=Mock(return_value=False))
        ...
        self.assertEqual(task.error_msg, "Not enough resources")
        eng.split_connector.send_cache_info_to_prefill.assert_called_once()
        self._detach_finalizer(eng)

    def test_decode_process_splitwise_requests_v1_prefilled_success_internal_adapter(self):
        cfg = self._make_cfg(splitwise_role="decode", num_gpu_blocks_override=4, router="0.0.0.0:30000")
        eng = self._make_engine(cfg)
        eng.running = True
        eng.enable_decode_cache_task = False
        ...
        eng.engine_worker_queue = Mock(
            disaggregate_queue_empty=Mock(side_effect=[False, True]),
            get_disaggregated_tasks=Mock(return_value=[(0, [out])]),
        )
        ...
        eng.scheduler.put_results.assert_called()
        eng.resource_manager.add_prefilled_request.assert_called_once()
        self.assertEqual(eng.token_processor.tokens_counter.get("rid_ok"), 1)
        self._detach_finalizer(eng)

    def test_decode_process_splitwise_requests_logs_loop_exception(self):
        cfg = self._make_cfg(splitwise_role="decode", num_gpu_blocks_override=4, router="0.0.0.0:30000")
        eng = self._make_engine(cfg)
        eng.running = True
        eng.enable_decode_cache_task = False
        ...
        eng.scheduler = Mock(has_request=Mock(return_value=True), put_results=Mock())
        eng.resource_manager = Mock(preallocate_resource_in_d=Mock(return_value=True))
        eng.engine_worker_queue = Mock(disaggregate_queue_empty=Mock(side_effect=RuntimeError("boom")))
        ...
        with (
            patch("fastdeploy.engine.common_engine.threading.Thread", DummyThread),
            patch("fastdeploy.engine.common_engine.time.sleep", side_effect=stop_after_error),
            patch.object(eng, "llm_logger") as mock_logger,
        ):
            eng._decode_process_splitwise_requests()
