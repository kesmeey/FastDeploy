...
def _disable_triton_cuda_path(monkeypatch):
    ...
    monkeypatch.setattr("fastdeploy.model_executor.layers.sample.sampler.batched_count_greater_than", lambda x, y: (x >= y).sum(-1))


@pytest.fixture
def mock_ops(monkeypatch):
    monkeypatch.setattr("fastdeploy.model_executor.layers.sample.sampler.apply_penalty_multi_scores", lambda *a, **k: a[1])
    monkeypatch.setattr(
...
def test_top_p_and_padding_sampling_params(monkeypatch):
    ...
    monkeypatch.setattr(paddle, "gather", lambda x, index, axis=0, name=None: original_gather(x.astype("int32") if x.dtype == paddle.bool else x, index, axis=axis, name=name))
    monkeypatch.setattr(paddle, "where", lambda cond, x=None, y=None, name=None: original_where(cond.astype("bool") if cond.dtype != paddle.bool else cond, x, y, name=name))
...
@pytest.mark.parametrize("mode,next_token,use_processor", [("processed_logprobs", 2, False), ("raw_logprobs", 1, True), ("processed_logits", 1, True)])
def test_sampler_forward_cuda_variants(mock_ops, monkeypatch, mode, next_token, use_processor):
    ...
    monkeypatch.setattr(
        ...
        lambda probs, top_p, top_k, top_k_list, topp_seed=None: (None, paddle.to_tensor([[next_token]], dtype="int64")),
    )
...
def test_sampler_init_and_intel_hpu(monkeypatch):
    ...
    out = sampler.forward_intel_hpu(paddle.ones([2, 4], dtype="float16"), m, paddle.to_tensor([0, 1], dtype="int64"), 3, 0, 0)
...
def test_speculative_sampler_basic(monkeypatch):
    fd_config = types.SimpleNamespace(
        ...
        speculative_config=types.SimpleNamespace(verify_window=2, max_candidate_len=4, benchmark_mode=False, enf_gen_phase_tag=False),
    )
    monkeypatch.setattr("fastdeploy.model_executor.layers.sample.sampler.current_platform.is_cuda", lambda: True)
    monkeypatch.setattr("fastdeploy.model_executor.layers.sample.sampler.current_platform.is_xpu", lambda: False)
    ...
    m.temp_scaled_logprobs_flag = True
    m.top_p_normalized_logprobs_flag = True
    m.share_inputs = {"seq_lens_this_time": paddle.to_tensor([[1]], dtype="int64"), "accept_num": paddle.to_tensor([1], dtype="int64")}
...
def test_mtp_sampler_xpu_and_compute(mock_ops, monkeypatch):
    ...
    monkeypatch.setattr("fastdeploy.model_executor.layers.sample.sampler.top_k_top_p_sampling", lambda *a, **k: (None, paddle.to_tensor([[1]], dtype="int64")))
    next_tokens, output = sampler.forward_xpu(paddle.ones([1, 4], dtype="float32"), m, max_model_len=8, share_inputs=m.share_inputs)
    ...
    m2.share_inputs = {"seq_lens_this_time": paddle.to_tensor([[1]], dtype="int64"), "batch_token_num": paddle.to_tensor([[1]], dtype="int64")}
    gathered = mtp.gather_logprobs(mtp.compute_logprobs(paddle.to_tensor([[1.0, 2.0]], dtype="float32"), m2), 0, paddle.to_tensor([1], dtype="int64"))
    assert gathered.logprob_token_ids.shape[1] == 1
