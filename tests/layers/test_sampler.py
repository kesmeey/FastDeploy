...
def test_speculative_sampler_forward_cuda(monkeypatch):
    sampler = _make_speculative_sampler()
    logits = paddle.ones([2, 4], dtype="float32")
    sampling_metadata = _create_default_sampling_metadata(
        batch_size=2, min_seq_len=1, max_seq_len=3, max_num_logprobs=1
    )
    sampling_metadata.seed = paddle.full([2, 1], 7, dtype="int64")
    sampling_metadata.share_inputs = _build_speculative_share_inputs(batch_size=2)
    sampling_metadata.top_k = paddle.full([2, 1], 3, dtype="int64")
    sampling_metadata.top_k_list = [3, 3]
    original_gather = paddle.gather
    original_where = paddle.where
    ...
    def _reasoning_phase_token_constraint(*args, **kwargs):
        return None
    ...
    def _top_p_candidates(*args, **kwargs):
        return paddle.ones([2, 1]), paddle.zeros([2, 1], dtype="int64"), paddle.to_tensor([1], dtype="int64")

    def _speculate_verify(*args, **kwargs):
        return None

    def _speculate_get_target_logits(target_logits, logits, *args, **kwargs):
        target_logits[:] = logits[: target_logits.shape[0], :]
    ...
    monkeypatch.setattr(
        "fastdeploy.model_executor.layers.sample.sampler.reasoning_phase_token_constraint",
        _reasoning_phase_token_constraint,
    )
    monkeypatch.setattr("fastdeploy.model_executor.layers.sample.sampler.top_k_top_p_sampling", _top_k_top_p_sampling)
    stub_gpu = types.SimpleNamespace(
        speculate_verify=_speculate_verify,
        top_p_candidates=_top_p_candidates,
    )
    monkeypatch.setitem(sys.modules, "fastdeploy.model_executor.ops.gpu", stub_gpu)
    monkeypatch.setattr(
        "fastdeploy.model_executor.layers.sample.sampler.speculate_get_target_logits",
        _speculate_get_target_logits,
    )

    output = sampler.forward_cuda(
