...
def test_triton_weight_only_create_and_apply(fake_ops, monkeypatch):
...
def test_python_op_fused_moe_kernel_paddle(fake_ops, monkeypatch):
...
def test_blockwise_create_weights_and_process(fake_ops, monkeypatch):
    quant_config = DummyQuantConfig(is_checkpoint_bf16=False, weight_block_size=(2, 2))
    layer = DummyLayer(quant_config)
...
def test_blockwise_process_weights_after_loading_bf16(fake_ops, monkeypatch):
...
def test_blockwise_check_and_apply_empty(fake_ops, monkeypatch):
    quant_config = DummyQuantConfig(is_checkpoint_bf16=False, weight_block_size=(2, 2))
    layer = DummyLayer(quant_config)
    method = backend.BlockWiseFP8MoEMethod(quant_config)
    method.create_weights(layer, model_format="torch")
